...
class _TxRow(QFrame):
    def __init__(self, tx, parent=None):
        ...
        icon = QLabel(tx["category_icon"] or "💰")
        icon.setFont(QFont("Segoe UI Emoji", 17))
        ...
        c.setStyleSheet("font-size:10px; color:#A0AABF;")
        info.addWidget(d)
        info.addWidget(c)
        lay.addLayout(info)
        lay.addStretch()
        ...
        dt = QLabel(tx["date"])
        dt.setStyleSheet("color:#A0AABF; font-size:10px;")
...
class DashboardPage(QWidget):
    ...
    def _build(self):
        ...
        self._date_lbl.setStyleSheet("color:#7A849E; font-size:13px;")
        hdr.addWidget(t)
        hdr.addStretch()
        ...
        rl.addWidget(self._sec("Transações Recentes"))
    ...
    def refresh(self):
        ...
        diff_str = f"{'↑' if dp > 0 else '↓'} {abs(dp):.1f}% vs mês anterior"
        ...
        for title, val, sub, icon, color in [
            ("Saldo do Mês", f"R$ {balance:,.2f}",
             MONTHS_PT[m], "💎", bc),
            ("Receitas", f"R$ {income:,.2f}",
             f"{sum(1 for t in txs if t['type']=='income')} transações",
             "📈", "#16A34A"),
            ("Despesas", f"R$ {expense:,.2f}",
             diff_str, "📉", "#DC2626"),
            ("Assinaturas", f"R$ {sub_total:,.2f}",
             f"{sub_count} ativas", "🔄", "#F59E0B"),
        ]:
            self._kpi_row.addWidget(
                KPICard(title, val, sub, icon, color))
        ...
        if cats:
            ...
            for r in cats[:5]:
                ...
                nm = QLabel(f"{r['icon']} {r['name']}")
                nm.setStyleSheet("font-size:11px;")
                vl = QLabel(f"R$ {r['total']:,.0f} ({pct:.0f}%)")
                vl.setStyleSheet("font-size:11px; color:#A0AABF;")
        ...
        if recent:
            for tx in recent:
                ...
                sep.setStyleSheet(
                    "background:#F0F4FF; max-height:1px;")
                self._recent_lay.addWidget(sep)
        else:
            e = QLabel("Nenhuma transação ainda")
            e.setStyleSheet("color:#A0AABF; padding:16px;")
        ...
        if active_subs:
            for s in active_subs[:6]:
                ...
                dy.setStyleSheet("font-size:10px; color:#A0AABF;")
                info_v.addWidget(nm)
                info_v.addWidget(dy)
                vl = QLabel(f"R$ {s['amount']:,.2f}")
                vl.setStyleSheet(
                    f"color:{s['color']}; font-weight:700; font-size:13px;")
                row_w.addWidget(ic)
